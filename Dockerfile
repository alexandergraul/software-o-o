...
# Update distro
RUN zypper --quite --non-interactive update

# Install some system requirements
RUN zypper --quite --non-interactive install timezone vim aaa_base glibc-locale sudo nodejs

# Install ruby
RUN zypper --quite --non-interactive install ruby2.5 ruby2.5-devel
